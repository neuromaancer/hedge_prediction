...
@Lab: CoML/Articulab
@School: PSL/ENS
@Description: Test XGboost classifier.
"""
...
@click.option(
    "-l",
    "--log_file",
    default="../logs/xgboost/xgboost.log",
    type=str,
    help="Name of the log file.",
)
def train_xgboost(k_folds, compute_shap, log_file):
    ...
    for sample in train_dataset:
        ...
        values = [t.reshape(-1).cpu() for t in sample.values()]
    ...
    rprint("[cyan]XGBoost classifier trained.")
    rprint("[cyan]Predicting...")
    y_pred = best_model.predict(dtest)
    y_pred = np.round(y_pred)

    rprint("[green]XGboost classifier trained.")
    rprint("[green]Predicting...")
    ...
    if compute_shap:
        ...
        reshaped_average_shap_values = all_shap_values.reshape((1287, -1, 51))
        # shap_vals_avg_1 = reshaped_average_shap_values[1]
        shap_vals_avg_1 = np.mean(reshaped_average_shap_values, axis=1)
        rprint(f"shap_vals_avg_1 shape: {shap_vals_avg_1.shape}")
        ...
        plt.savefig(save_to / f"xgboost_shap_{today}_summary_1.png")
